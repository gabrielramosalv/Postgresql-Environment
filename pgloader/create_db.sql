...
CREATE TABLE IF NOT EXISTS instituicoes.instituicao_stg (
...
CREATE TABLE instituicoes.situacao (
...
CREATE TABLE instituicoes.local_funcionamento (
    id BIGSERIAL PRIMARY KEY,
    nome VARCHAR(50) NOT NULL UNIQUE
);
...
CREATE TABLE instituicoes.modalidade (
    id BIGSERIAL PRIMARY KEY,
    nome VARCHAR(50) NOT NULL UNIQUE
);

DROP TABLE IF EXISTS turno CASCADE;
CREATE TABLE instituicoes.turno (
    id BIGSERIAL PRIMARY KEY,
    nome VARCHAR(20) NOT NULL UNIQUE
);

DROP TABLE IF EXISTS dependencia_fisica CASCADE;
CREATE TABLE instituicoes.dependencia_fisica (
    id BIGSERIAL PRIMARY KEY,
    nome VARCHAR(100) NOT NULL UNIQUE
);

DROP TABLE IF EXISTS empresa_tipo CASCADE;
CREATE TABLE instituicoes.empresa_tipo (
    id BIGSERIAL PRIMARY KEY,
    nome VARCHAR(100) NOT NULL UNIQUE
);

DROP TABLE IF EXISTS entidade_tipo CASCADE;
CREATE TABLE instituicoes.entidade_tipo (
    id BIGSERIAL PRIMARY KEY,
    nome VARCHAR(50) NOT NULL UNIQUE
);

DROP TABLE IF EXISTS tipo_relacao_instituicao CASCADE;
CREATE TABLE instituicoes.tipo_relacao_instituicao (
    id BIGSERIAL PRIMARY KEY,
    nome VARCHAR(50) NOT NULL
);

DROP TABLE IF EXISTS recurso_tipo CASCADE;
CREATE TABLE instituicoes.recurso_tipo (
    id BIGSERIAL PRIMARY KEY,
    nome VARCHAR(50) NOT NULL UNIQUE
);

DROP TABLE IF EXISTS segmento CASCADE;
CREATE TABLE instituicoes.segmento (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE instituicoes.segmento_indicador_tipo (
   id BIGSERIAL PRIMARY KEY,
   nome VARCHAR(50) NOT NULL UNIQUE
);
...
CREATE TABLE instituicoes.pais (
    id BIGSERIAL PRIMARY KEY,
    nome VARCHAR(50) NOT NULL UNIQUE
);

DROP TABLE IF EXISTS regiao CASCADE;
CREATE TABLE instituicoes.regiao (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE instituicoes.estado (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE instituicoes.municipio (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE instituicoes.regiao_intermediaria (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE instituicoes.regiao_imediata (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE instituicoes.bairro (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE instituicoes.endereco (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE instituicoes.empresa (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE instituicoes.orgao (
    id BIGSERIAL PRIMARY KEY,
    nome VARCHAR(50) NOT NULL UNIQUE
);
...
CREATE TABLE instituicoes.instituicao (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE instituicoes.local_funcionamento_instituicao (
...
CREATE TABLE instituicoes.dependencia_fisica_instituicao (
    id_instituicao BIGINT NOT NULL REFERENCES instituicao(id),
...
CREATE TABLE instituicoes.relacao_instituicao (
    id_instituicao BIGINT NOT NULL REFERENCES instituicao(id),
...
CREATE TABLE instituicoes.exame (
    id BIGSERIAL PRIMARY KEY,
    id_instituicao BIGINT NOT NULL REFERENCES instituicao(id)
);
...
-- Situação
INSERT INTO instituicoes.situacao (id, nome) VALUES
(1, 'Em Atividade'),
(2, 'Paralisada'),
...
-- Tipo de empresa
INSERT INTO instituicoes.empresa_tipo (id, nome) VALUES
...
-- Órgãos
INSERT INTO instituicoes.orgao (id, nome) VALUES
...
-- Tipos de entidade
INSERT INTO instituicoes.entidade_tipo (id, nome) VALUES
(1, 'Empresa'),
...
INSERT INTO instituicoes.tipo_relacao_instituicao (id, nome) VALUES
(1, 'Vínculo'),
(2, 'Mantenedora'),
(3, 'Sediada'),
(4, 'Sedia');
...
-- Turnos
INSERT INTO instituicoes.turno (id, nome) VALUES
(1, 'Diurno'),
(2, 'Noturno'),
(3, 'EAD');

-- Modalidade
INSERT INTO instituicoes.modalidade (id, nome) VALUES
(1, 'Presencial'),
(2, 'Semipresencial'),
(3, 'EAD');

-- Segmentos
INSERT INTO instituicoes.segmento (id, nome) VALUES
(1, 'Ensino Infantil - Creche'),
(2, 'Ensino Infantil - Pré-Escola'),
(3, 'Ensino Fundamental - Anos Iniciais (1° ano até o 5 ano°)'),
...
INSERT INTO instituicoes.segmento (id, nome, descricao) VALUES
...
INSERT INTO instituicoes.reserva_vaga_tipo (id, nome) VALUES
...
INSERT INTO instituicoes.local_funcionamento (id, nome) VALUES
(1, 'Prédio escolar'),
...
INSERT INTO instituicoes.dependencia_fisica (id, nome) VALUES
(1, 'Almoxarifado'),
...
BEGIN
    ...
    -- País
    SELECT id INTO v_id_pais FROM instituicoes.pais WHERE nome = 'Brasil';
    IF NOT FOUND THEN
        INSERT INTO instituicoes.pais (nome) VALUES ('Brasil') RETURNING id INTO v_id_pais;
    END IF;

    -- Região
    SELECT id INTO v_id_regiao FROM instituicoes.regiao WHERE codigo = NEW.CO_REGIAO;
    IF NOT FOUND THEN
        INSERT INTO instituicoes.regiao (id_pais, codigo, nome)
        VALUES (v_id_pais, NEW.CO_REGIAO, NEW.NO_REGIAO)
        RETURNING id INTO v_id_regiao;
    END IF;

    -- Estado
    SELECT id INTO v_id_estado FROM instituicoes.estado WHERE codigo = NEW.CO_UF;
    IF NOT FOUND THEN
        INSERT INTO instituicoes.estado (id_regiao, codigo, nome)
        VALUES (v_id_regiao, NEW.CO_UF, NEW.NO_UF)
        RETURNING id INTO v_id_estado;
    END IF;

    -- Município
    SELECT id INTO v_id_municipio FROM instituicoes.municipio WHERE sigla = NEW.SG_UF;
    IF NOT FOUND THEN
        INSERT INTO instituicoes.municipio (id_estado, sigla, nome)
        VALUES (v_id_estado, NEW.SG_UF, NEW.NO_MUNICIPIO)
        RETURNING id INTO v_id_municipio;
    END IF;

    -- Região intermediária
    SELECT id INTO v_id_regiao_intermediaria FROM instituicoes.regiao_intermediaria WHERE codigo = NEW.CO_REGIAO_GEOG_INTERM;
    IF NOT FOUND THEN
        INSERT INTO instituicoes.regiao_intermediaria (id_municipio, codigo, nome)
        VALUES (v_id_municipio, NEW.CO_REGIAO_GEOG_INTERM, NEW.NO_REGIAO_GEOG_INTERM)
        RETURNING id INTO v_id_regiao_intermediaria;
    END IF;

    -- Região imediata
    SELECT id INTO v_id_regiao_imediata FROM instituicoes.regiao_imediata WHERE codigo = NEW.CO_REGIAO_GEOG_IMED;
    IF NOT FOUND THEN
        INSERT INTO instituicoes.regiao_imediata (id_reg_inter, codigo, nome)
    ...
    IF NEW.NO_BAIRRO IS NOT NULL THEN
        SELECT id INTO v_id_bairro FROM instituicoes.bairro WHERE nome = NEW.NO_BAIRRO;
        IF NOT FOUND THEN
            INSERT INTO instituicoes.bairro (id_reg_imed, id_municipio, nome)
    ...
    -- Endereço
    INSERT INTO instituicoes.endereco (id_bairro, cep, endereco, referencia, complemento)
    ...
    UPDATE instituicoes.endereco SET id_instituicao = v_id_instituicao WHERE id = v_id_endereco;
    ...
    IF v_cnpj IS NOT NULL THEN
        ...
        SELECT id INTO v_temp_id FROM instituicoes.empresa WHERE cnpj = NEW.NU_CNPJ_MANTENEDORA;
        IF NOT FOUND THEN
            INSERT INTO instituicoes.empresa (id_empresa_tipo, cnpj) values (v_empresa_tipo_id, NEW.NU_CNPJ_MANTENEDORA) RETURNING id INTO v_temp_id;
        END IF;

        INSERT INTO instituicoes.relacao_instituicao (id_instituicao, id_entidade, id_entidade_tipo, id_tipo_relacao_instituicao) values (v_id_instituicao, v_temp_id, 1, 2);
    END IF;
    
    -- Instituição pública mantenedora (se existir)
    IF NEW.TP_DEPENDENCIA = 1 THEN
    	INSERT INTO instituicoes.relacao_instituicao (id_instituicao, id_entidade, id_entidade_tipo, id_tipo_relacao_instituicao) values (v_id_instituicao, v_id_pais, 6, 2);
    ELSIF NEW.TP_DEPENDENCIA = 2 THEN
    	INSERT INTO instituicoes.relacao_instituicao (id_instituicao, id_entidade, id_entidade_tipo, id_tipo_relacao_instituicao) values (v_id_instituicao, v_id_estado, 5, 2);
    ELSIF NEW.TP_DEPENDENCIA = 3 THEN
    	INSERT INTO instituicoes.relacao_instituicao (id_instituicao, id_entidade, id_entidade_tipo, id_tipo_relacao_instituicao) values (v_id_instituicao, v_id_municipio, 4, 2);
    END IF;
    
    -- Orgãos
    IF NEW.IN_VINCULO_SECRETARIA_EDUCACAO = 1 THEN
        INSERT INTO instituicoes.relacao_instituicao (id_instituicao, id_entidade, id_entidade_tipo, id_tipo_relacao_instituicao) VALUES (v_id_instituicao, 1, 2, 1);
    END IF;
    IF NEW.IN_VINCULO_SEGURANCA_PUBLICA = 1 THEN
        INSERT INTO instituicoes.relacao_instituicao (id_instituicao, id_entidade, id_entidade_tipo, id_tipo_relacao_instituicao) VALUES (v_id_instituicao, 2, 2, 1);
    END IF;
    IF NEW.IN_VINCULO_SECRETARIA_SAUDE = 1 THEN
        INSERT INTO instituicoes.relacao_instituicao (id_instituicao, id_entidade, id_entidade_tipo, id_tipo_relacao_instituicao) VALUES (v_id_instituicao, 3, 2, 1);
    END IF;
    IF NEW.IN_VINCULO_OUTRO_ORGAO = 1 THEN
        INSERT INTO instituicoes.relacao_instituicao (id_instituicao, id_entidade, id_entidade_tipo, id_tipo_relacao_instituicao) VALUES (v_id_instituicao, 4, 2, 1);
    END IF;
	...
	IF NEW.IN_DIURNO = 1 THEN
		INSERT INTO instituicoes.turno_instituicao (id_instituicao, id_turno) values (v_id_instituicao, 1);
	END IF;
	IF NEW.IN_NOTURNO = 1 THEN
		INSERT INTO instituicoes.turno_instituicao (id_instituicao, id_turno) values (v_id_instituicao, 2);
	END IF;
	IF NEW.IN_EAD = 1 THEN
		INSERT INTO instituicoes.turno_instituicao (id_instituicao, id_turno) values (v_id_instituicao, 3);
	END IF;
	...
	IF NEW.IN_INF_CRE = 1 THEN
		INSERT INTO instituicoes.segmento_instituicao (id_instituicao, id_segmento) VALUES (v_id_instituicao, 1) RETURNING id INTO v_temp_id;

	    IF NEW.QT_MAT_INF_CRE IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 1, NEW.QT_MAT_INF_CRE);
	    END IF;
	    IF NEW.QT_DOC_INF_CRE IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 2, NEW.QT_DOC_INF_CRE);
        END IF;
        IF NEW.QT_TUR_INF_CRE_INT IS NOT NULL THEN
           INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 3, NEW.QT_TUR_INF_CRE_INT);
        END IF;
	END IF;
	IF NEW.IN_INF_PRE = 1 THEN
		INSERT INTO instituicoes.segmento_instituicao (id_instituicao, id_segmento) VALUES (v_id_instituicao, 2) RETURNING id INTO v_temp_id;

	    IF NEW.QT_MAT_INF_PRE IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 1, NEW.QT_MAT_INF_PRE);
	    END IF;
	    IF NEW.QT_DOC_INF_PRE IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 2, NEW.QT_DOC_INF_PRE);
        END IF;
        IF NEW.QT_TUR_INF_PRE_INT IS NOT NULL THEN
           INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 3, NEW.QT_TUR_INF_PRE_INT);
        END IF;
	END IF;
	IF NEW.IN_FUND_AI = 1 THEN
		INSERT INTO instituicoes.segmento_instituicao (id_instituicao, id_segmento) VALUES (v_id_instituicao, 3) RETURNING id INTO v_temp_id;

	    IF NEW.QT_MAT_FUND_AI IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 1, NEW.QT_MAT_FUND_AI);
	    END IF;
	    IF NEW.QT_DOC_FUND_AI IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 2, NEW.QT_DOC_FUND_AI);
        END IF;
        IF NEW.QT_TUR_FUND_AI IS NOT NULL THEN
           INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 3, NEW.QT_TUR_FUND_AI);
        END IF;
	END IF;
	IF NEW.IN_FUND_AF = 1 THEN
		INSERT INTO instituicoes.segmento_instituicao (id_instituicao, id_segmento) VALUES (v_id_instituicao, 4) RETURNING id INTO v_temp_id;

	    IF NEW.QT_MAT_FUND_AF IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 1, NEW.QT_MAT_FUND_AF);
	    END IF;
	    IF NEW.QT_DOC_FUND_AF IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 2, NEW.QT_DOC_FUND_AF);
        END IF;
        IF NEW.QT_TUR_FUND_AF IS NOT NULL THEN
           INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 3, NEW.QT_TUR_FUND_AF);
        END IF;
	END IF;
	IF NEW.IN_EJA_FUND = 1 THEN
		INSERT INTO instituicoes.segmento_instituicao (id_instituicao, id_segmento) VALUES (v_id_instituicao, 5) RETURNING id INTO v_temp_id;

	    IF NEW.QT_MAT_EJA_FUND IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 1, NEW.QT_MAT_EJA_FUND);
	    END IF;
	    IF NEW.QT_DOC_EJA_FUND IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 2, NEW.QT_DOC_EJA_FUND);
        END IF;
        IF NEW.QT_TUR_EJA_FUND IS NOT NULL THEN
           INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 3, NEW.QT_TUR_EJA_FUND);
        END IF;
	END IF;
	IF NEW.IN_MED = 1 THEN
		INSERT INTO instituicoes.segmento_instituicao (id_instituicao, id_segmento) VALUES (v_id_instituicao, 6) RETURNING id INTO v_temp_id;

	    IF NEW.QT_MAT_MED IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 1, NEW.QT_MAT_MED);
	    END IF;
	    IF NEW.QT_DOC_MED IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 2, NEW.QT_DOC_MED);
        END IF;
        IF NEW.QT_TUR_MED IS NOT NULL THEN
           INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 3, NEW.QT_TUR_MED);
        END IF;
	END IF;
	IF NEW.IN_EJA_MED = 1 THEN
		INSERT INTO instituicoes.segmento_instituicao (id_instituicao, id_segmento) VALUES (v_id_instituicao, 7) RETURNING id INTO v_temp_id;

	    IF NEW.QT_MAT_EJA_MED IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 1, NEW.QT_MAT_EJA_MED);
	    END IF;
	    IF NEW.QT_DOC_EJA_MED IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 2, NEW.QT_DOC_EJA_MED);
        END IF;
        IF NEW.QT_TUR_EJA_MED IS NOT NULL THEN
           INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 3, NEW.QT_TUR_EJA_MED);
        END IF;
	END IF;
	IF NEW.IN_PROF = 1 THEN
		INSERT INTO instituicoes.segmento_instituicao (id_instituicao, id_segmento) VALUES (v_id_instituicao, 8) RETURNING id INTO v_temp_id;

	    IF NEW.QT_MAT_PROF IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 1, NEW.QT_MAT_PROF);
	    END IF;
	    IF NEW.QT_DOC_PROF IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 2, NEW.QT_DOC_PROF);
        END IF;
        IF NEW.QT_TUR_PROF IS NOT NULL THEN
           INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 3, NEW.QT_TUR_PROF);
        END IF;
	END IF;
	IF NEW.IN_PROF_TEC = 1 THEN
		INSERT INTO instituicoes.segmento_instituicao (id_instituicao, id_segmento) VALUES (v_id_instituicao, 9) RETURNING id INTO v_temp_id;

	    IF NEW.QT_MAT_PROF_TEC IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 1, NEW.QT_MAT_PROF_TEC);
	    END IF;
	    IF NEW.QT_DOC_PROF_TEC IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 2, NEW.QT_DOC_PROF_TEC);
        END IF;
        IF NEW.QT_TUR_PROF_TEC IS NOT NULL THEN
           INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 3, NEW.QT_TUR_PROF_TEC);
        END IF;
	END IF;
	IF NEW.IN_ESP_CC = 1 THEN
		INSERT INTO instituicoes.segmento_instituicao (id_instituicao, id_segmento) VALUES (v_id_instituicao, 10) RETURNING id INTO v_temp_id;

	    IF NEW.QT_MAT_ESP_CC IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 1, NEW.QT_MAT_ESP_CC);
	    END IF;
	    IF NEW.QT_DOC_ESP_CC IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 2, NEW.QT_DOC_ESP_CC);
        END IF;
        IF NEW.QT_TUR_ESP_CC IS NOT NULL THEN
           INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 3, NEW.QT_TUR_ESP_CC);
        END IF;
	END IF;
	IF NEW.IN_ESP_CE = 1 THEN
		INSERT INTO instituicoes.segmento_instituicao (id_instituicao, id_segmento) VALUES (v_id_instituicao, 11) RETURNING id INTO v_temp_id;

	    IF NEW.QT_MAT_ESP_CE IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 1, NEW.QT_MAT_ESP_CE);
	    END IF;
	    IF NEW.QT_DOC_ESP_CE IS NOT NULL THEN
	       INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 2, NEW.QT_DOC_ESP_CE);
        END IF;
        IF NEW.QT_TUR_ESP_CE IS NOT NULL THEN
           INSERT INTO instituicoes.segmento_indicador_instituicao (id_segmento_instituicao, id_segmento_indicador_tipo, valor) VALUES (v_temp_id, 3, NEW.QT_TUR_ESP_CE);
        END IF;
	END IF;
	...
	IF NEW.IN_MEDIACAO_PRESENCIAL = 1 THEN
		INSERT INTO instituicoes.modalidade_instituicao (id_instituicao, id_modalidade) VALUES (v_id_instituicao, 1);
	END IF;
	IF NEW.IN_MEDIACAO_SEMIPRESENCIAL = 1 THEN
		INSERT INTO instituicoes.modalidade_instituicao (id_instituicao, id_modalidade) VALUES (v_id_instituicao, 2);
	END IF;
	IF NEW.IN_MEDIACAO_EAD = 1 THEN
		INSERT INTO instituicoes.modalidade_instituicao (id_instituicao, id_modalidade) VALUES (v_id_instituicao, 3);
	END IF;
    ...
    FOR v_dependencia_id IN
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 1 AND NEW.IN_ALMOXARIFADO = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 2 AND NEW.IN_AREA_PLANTIO = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 3 AND NEW.IN_AUDITORIO = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 4 AND NEW.IN_BANHEIRO = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 5 AND NEW.IN_BANHEIRO_EI = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 6 AND NEW.IN_BANHEIRO_PNE = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 7 AND NEW.IN_BANHEIRO_FUNCIONARIOS = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 8 AND NEW.IN_BANHEIRO_CHUVEIRO = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 9 AND NEW.IN_BIBLIOTECA = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 10 AND NEW.IN_BIBLIOTECA_SALA_LEITURA = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 11 AND NEW.IN_COZINHA = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 12 AND NEW.IN_DESPENSA = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 13 AND NEW.IN_DORMITORIO_ALUNO = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 14 AND NEW.IN_DORMITORIO_PROFESSOR = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 15 AND NEW.IN_LABORATORIO_CIENCIAS = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 16 AND NEW.IN_LABORATORIO_INFORMATICA = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 17 AND NEW.IN_LABORATORIO_EDUC_PROF = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 18 AND NEW.IN_PATIO_COBERTO = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 19 AND NEW.IN_PATIO_DESCOBERTO = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 20 AND NEW.IN_PARQUE_INFANTIL = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 21 AND NEW.IN_PISCINA = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 22 AND NEW.IN_QUADRA_ESPORTES = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 23 AND NEW.IN_QUADRA_ESPORTES_COBERTA = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 24 AND NEW.IN_QUADRA_ESPORTES_DESCOBERTA = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 25 AND NEW.IN_REFEITORIO = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 26 AND NEW.IN_SALA_ATELIE_ARTES = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 27 AND NEW.IN_SALA_MUSICA_CORAL = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 28 AND NEW.IN_SALA_ESTUDIO_DANCA = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 29 AND NEW.IN_SALA_MULTIUSO = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 30 AND NEW.IN_SALA_ESTUDIO_GRAVACAO = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 31 AND NEW.IN_SALA_OFICINAS_EDUC_PROF = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 32 AND NEW.IN_SALA_DIRETORIA = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 33 AND NEW.IN_SALA_LEITURA = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 34 AND NEW.IN_SALA_PROFESSOR = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 35 AND NEW.IN_SALA_REPOUSO_ALUNO = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 36 AND NEW.IN_SECRETARIA = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 37 AND NEW.IN_SALA_ATENDIMENTO_ESPECIAL = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 38 AND NEW.IN_TERREIRAO = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 39 AND NEW.IN_VIVEIRO = 1 UNION
        SELECT id FROM instituicoes.dependencia_fisica WHERE id = 40 AND NEW.IN_DEPENDENCIAS_OUTRAS = 1
    LOOP
        INSERT INTO instituicoes.dependencia_fisica_instituicao (id_instituicao, id_dependencia_fisica)
...
DROP TRIGGER IF EXISTS trigger_distribuir_dados ON instituicoes.instituicao_stg;
CREATE TRIGGER trigger_distribuir_dados
AFTER INSERT ON instituicoes.instituicao_stg
FOR EACH ROW
EXECUTE FUNCTION distribuir_dados_instituicao();

DROP PROCEDURE IF EXISTS instituicoes.inserir_relacoes_sede();
CREATE OR REPLACE PROCEDURE instituicoes.inserir_relacoes_sede()
LANGUAGE plpgsql
AS $$
BEGIN
    -- Insere a relação "Sedia" (tipo 4) da sede para a vinculada
    INSERT INTO instituicoes.relacao_instituicao (
        id_instituicao,
        id_entidade,
        id_entidade_tipo,
    ...
    SELECT
        ...
        3,                    
        4                     
    FROM instituicoes.instituicao vinculada
    JOIN instituicoes.instituicao sede
        ON vinculada.codigo_sede IS NOT NULL
        AND vinculada.codigo_sede <> vinculada.codigo
        AND vinculada.codigo_sede = sede.codigo
    WHERE NOT EXISTS (
        SELECT 1 FROM instituicoes.relacao_instituicao ir
    ...
    ALTER TABLE instituicoes.instituicao DROP COLUMN IF EXISTS codigo_sede;
